...
resource "aws_instance" "kafka_controller" {
  ...
  tags = {
    Name = "${var.user}-kafka-controller-${count.index + 1}"
...
resource "aws_instance" "kafka_broker" {
  ...
  tags = {
    Name = "${var.user}-kafka-broker-${count.index + 1}"
    Role = "kafka_broker"
  }
}
...
# Control Center with Internet Access
resource "aws_instance" "control_center" {
  ...
  tags = {
    Name = "${var.user}-control-center-${count.index + 1}"
    Role = "control_center"
  }
}
...
resource "aws_instance" "schema_registry" {
  ...
  tags = {
    Name = "${var.user}-schema-registry-${count.index + 1}"
...
resource "aws_instance" "ksql" {
  ...
  tags = {
    Name = "${var.user}-ksql-${count.index + 1}"
    Role = "ksql"
  }
}
